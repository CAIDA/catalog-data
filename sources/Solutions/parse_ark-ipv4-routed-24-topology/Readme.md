...
Each line indicates that a link `link_id` connects nodes N<sub>1</sub> to N<sub>m</sub>. If it is known which router interface is connected to the link, then the interface address is given after the node ID separated by a colon.
~~~
